...
- `validator.py`: Contains the function `validate_numbers`, which checks if the provided numbers are valid before performing an operation. However, this file tries to use calculation functions (like `multiply_numbers`), creating a cycle.

3. **Separate the logic into different files.** To avoid circular imports, we will reorganize the code.
...
- Now, in `validator.py`, we will keep only the validation logic. We don't need the calculation functions here. Example:
...
4. **Refactor the `app.py` file.** Import the calculation functions from `utils.py` and the validation function from `validator.py`.
...
- If they are valid, perform addition and multiplication.
